...
$url = $null
$logFile = Join-Path $root 'logs\cpolar\cpolar-tunnel.log'
if (Test-Path $logFile) {
    $log = Get-Content $logFile -Encoding UTF8 -ErrorAction SilentlyContinue
    $url = ($log | Select-String -Pattern 'https://\S+\.cpolar\.\w+' | Select-Object -Last 1).Matches.Value
}
if ($url) {
    ...
    Write-Host "       未在 $logFile 找到隧道地址" -ForegroundColor Red
    Write-Host '       请确认已运行：start-external.ps1 或 cpolar http 8080' -ForegroundColor Yellow
}
