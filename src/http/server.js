  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (url.pathname.startsWith('/api/')) {
        return await handleApi(req, res, url, server);
      }
      if (url.pathname === '/healthz') {
        return json(res, 200, { ok: true, ts: Date.now(), rooms: server.rooms?.roomCount ?? 0 });
      }
      ...
      logger.error('http', '处理请求异常', { path: req.url, error: err.message });
...
async function handleApi(req, res, url, server) {
  ...
  if (p === '/auth/register' && req.method === 'POST') {
    ...
    if (error) return json(res, 400, { error, message: '注册失败' });
  ...
  if (p === '/auth/login' && req.method === 'POST') {
    ...
    const { user, error } = userApi.loginUser(body.name, body.password);
    if (error) return json(res, 401, { error, message: '昵称或密码错误' });
  ...
  if (p === '/admin/users' && req.method === 'POST') {
    // 管理后台：创建用户
    const body = await readBody(req);
    const { user, error } = userApi.adminCreateUser(body.name, body.password);
    if (error) return json(res, 400, { error, message });
...
async function serveAdminPage(res) {
  try {
    ...
    logger.error('http', '管理页加载失败', { error: err.message });
