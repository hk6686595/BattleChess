...
export function colorOfTurn(turn) {
...
export function parseMove(raw) {
  ...
  const src =
    Number.isInteger(Number(raw.x)) && Number.isInteger(Number(raw.y))
      ? raw
 ...
 */
export function findWinLine(board, x, y) {
...
export function undoLastMove(state) {
  ...
  state.lastMove = prev
    ? lastMoveView(prev.x ?? prev.to?.x, prev.y ?? prev.to?.y)
    : null;
  state.over = false;
  state.winnerId = null;
  state.isDraw = false;
...
export function serialize(state) {
  return {
    ...
    lastMove: state.lastMove
      ? lastMoveView(state.lastMove.x, state.lastMove.y)
      : null,
