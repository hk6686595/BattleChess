...
export function toUcciMove(from, to) {
  const file = (x) => 'abcdefghi'[x];
  return `${file(from.x)}${9 - from.y}${file(to.x)}${9 - to.y}`;
}

/** UCCI 走法 -> 内部坐标（y 反转回内部视角） */
export function fromUcciMove(mv) {
  if (!/^[a-i][0-9][a-i][0-9]$/.test(mv)) return null;
  const f = (ch) => 'abcdefghi'.indexOf(ch);
  return {
 ...
 */
export function boardToFen(board, side = 'r') {
  const rows = [];
  for (let y = 0; y < 10; y++) {
    let row = '';
    let empty = 0;
...
class UcciEngine {
   ...
   */
  async getBestMove(board, side = 'b', timeMs = 1500) {
    ...
    try {
      ...
      logger.warn('engine', '引擎计算失败', { error: err.message });
      return null;
    } finally {
      this.busy = false;
    }
